...
(async function main() {
  ...
  try {
    ...
    const filesResult = await fetch(`https://api.figma.com/v1/files/${FIGMA_PROJECT_ID}`, fetchOptions);
    const files = await filesResult.json();

    const canvasIds = files.document.children.filter((child) => child.type === 'CANVAS').map((child) => child.id);
    spinner.succeed(`Retrieved ${canvasIds.length} pages for project ${FIGMA_PROJECT_ID}`);
    ...
    const nodesUrl = new URL(`https://api.figma.com/v1/files/${FIGMA_PROJECT_ID}/nodes`);
    ...
    spinner.succeed(`Retrieved ${exportNodes.length} total export nodes across all pages`);

    // Fetch a list of URLs to download the nodes from in SVG format.
    spinner.start(`Retrieving image URLs for nodes.`);
    const imagesUrl = new URL(`https://api.figma.com/v1/images/${FIGMA_PROJECT_ID}/`);
    ...
    spinner.succeed(`Retrieved ${Object.entries(imageUrls.images).length} image Urls`);
