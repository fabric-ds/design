...
1. Utility classes
2. Customised subset of Tailwind classes

## Lets make a FINN page

So lets pretend we get a task to create a FINN page, we will use the [Min FINN](https://www.finn.no/min-finn) page as an
example.

We will dig a little deeper into the main menu part of it.
![Screenshot of minfinn, the main menu](guide-img/min-finn-menu.png) The mobile version:
...
React/Vue/Element cards? Maybe choose one and show how its done? [Card in React](https://react.fabric-ds.io/card)
[Card in Vue](https://vue.fabric-ds.io/card) [Card in Element](https://elements.fabric-ds.io/pages/components/card.html)

### 6. Card content

![Screenshot of minfinn, one item from the main menu](guide-img/min-finn-card.png) Looking at the layout for each card
we would need a little bit of markup to start with, something like:
