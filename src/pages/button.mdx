...
### Flexible width
The width of a button automatically adjusts to fit the text. The default padding on each side of the button is set to 16px, wit the only exception to the rule being when buttons are used as full-width on mobile or in other containers when a full-width button makes sense.
